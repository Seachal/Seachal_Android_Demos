...
import com.seachal.seachaltest.bean.CourseJumpBeanNotNull
...
//        textView.text = "hello"
        textView.setOnClickListener {
            try {
                val str =
                    "xkw://train/native/?targetType=172&params={\"course\":\"978593720889311232\",\"chapter\": \"978596699155529728\"}"
//               String 转 uri。
                ...
                val chapter =
                    jsonObject.getLong("chapter") // getString  //  当值值对时， "978596699155529728" 可以解析正常， "" 用 getLong 接收会报异常
...
//                测试一下会不会报异常
                ...
                val uri3: Uri? =
                    Uri.parse(null) // 会报异常 ，    private StringUri(String uriString) {if (uriString == null) {throw new NullPointerException("uriString"); }
...
//        textView2.text = "hello"
        textView2.setOnClickListener {
            try {
                val str =
                    "xkw://train/native/?targetType=172&params={\"course\":\"978593720889311232\",\"chapter\": \"\"}"
//               String 转 uri。
        ...
        val textView_gson_not_null: TextView = findViewById<TextView>(R.id.textView_gson_not_null)
        textView_gson_not_null.setOnClickListener {
            try {
...
//  修改params={"
                val str =
                    "xkw://train/native/?targetType=172&params={\"courseId\":\"978593720889311232\",\"chapterId\": \"\"}"
                val uri: Uri? = Uri.parse(str)
                val targetType = uri?.getQueryParameter("targetType")
                //  是一个json  字符串
                val jsonString = uri?.getQueryParameter("params")
                val data =
                    GsonUtil.toBean<CourseJumpBeanNotNull>(jsonString ?: "", CourseJumpBeanNotNull::class.java)

                val text =
                    "targetType=${targetType},course=${data?.courseId},chapter=${data?.chapterId}"

                textView_gson_not_null.text = text
            } catch (e: Exception) {
                Log.e("StringToUriActivity", e.printStackTrace().toString())
                textView_gson_not_null.text = e.message
            }
...
//  修改params={"
                ...
                /*  val gson = Gson()
                  ...
                  */
                val data =
                    GsonUtil.toBean<CourseJumpBean>(jsonString ?: "", CourseJumpBean::class.java)
...
//  修改params={"
                ...
                /*  val gson = Gson()
                  val data = gson.fromJson(jsonString, CourseJumpBean::class.java)
                  */
                val data = GsonUtil.toBean<CourseJumpBeanString>(
                    jsonString ?: "",
                    CourseJumpBeanString::class.java
                )
